...
MQTTNetworkCallback(UA_ConnectionManager *tcpCM, uintptr_t connectionId,
    ...
    if(state == UA_CONNECTIONSTATE_ESTABLISHED &&
       oldState != UA_CONNECTIONSTATE_ESTABLISHED) {
        ...
        const char *mqtt_client_id = getenv("IOT_DEVICE_ID");
        if(!mqtt_client_id) mqtt_client_id = getenv("AZURE_DEVICE_ID");
        if(!mqtt_client_id) mqtt_client_id = "device2";
        
        const char *iot_hub_name = getenv("IOT_HUB_NAME");
        if(!iot_hub_name) iot_hub_name = getenv("AZURE_IOT_HUB_NAME");
        if(!iot_hub_name) iot_hub_name = "iot63018734.azure-devices.net";
        
        static char mqtt_username[256];
        snprintf(mqtt_username, sizeof(mqtt_username), "%s/%s/?api-version=2021-04-12", iot_hub_name, mqtt_client_id);

        enum MQTTErrors err = mqtt_connect(&bc->client, mqtt_client_id, NULL, NULL, 0,
                                           mqtt_username, NULL, MQTT_CONNECT_CLEAN_SESSION, 400);
        if(err != MQTT_OK) {
...
MQTT_sendWithConnection(UA_ConnectionManager *cm, uintptr_t connectionId,
    ...
    UA_LOG_INFO(bc->mcm->cm.eventSource.eventLoop->logger,
                 UA_LOGCATEGORY_NETWORK, "DEBUG_MQTT_PUBLISH: topic='%s', payload_size=%u, payload='%.*s'",
                 (char*)tc->topic.data, (unsigned)buf->length, (int)buf->length, (char*)buf->data);
